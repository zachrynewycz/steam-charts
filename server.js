...
app.get("/getTopGames", async (req, res) => {
    ...
    const gameIds = await page.$$eval("#top-games > tbody > tr > td.game-name.left > a", (el) =>
        el.map((td) => {
            return td.getAttribute("href").split("/")[2];
        })
    );
    const gameNames = await page.$$eval("#top-games > tbody > tr > td.game-name.left > a", (el) =>
        el.map((td) => {
            return td.innerText;
        })
    );
    const currentPlayers = await page.$$eval("#top-games > tbody > tr > td:nth-child(3)", (el) =>
        el.map((td) => {
            return td.innerText;
        })
    );
    const peakPlayers = await page.$$eval("#top-games > tbody > tr > td.num.period-col.peak-concurrent", (el) =>
        el.map((td) => {
            return td.innerText;
        })
    );

    const gameData = gameIds.map((id, index) => {
        return {
            name: gameNames[index],
            id: id,
            current: currentPlayers[index],
            peak: peakPlayers[index],
        };
    });
...
app.get("/getRecords", async (req, res) => {
    ...
    const appids = await page.$$eval("#toppeaks > tbody > tr> td.game-name.left > a", (e) =>
        e.map((td) => {
            return td.getAttribute("href").replace(/\D/g, "");
        })
    );

    const records = appids.map((id, index) => {
        return { id, data: game[index] };
    });
